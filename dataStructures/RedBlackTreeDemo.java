...
public class RedBlackTreeDemo {
	...
	public static void testFromBook() {
		...
		t.add("D");
		System.out.println("TestFromBook add ROOT: " + t.root.data.toString()
				+ ": " + t.cString());
		t.add("B");
		System.out.println("TestFromBook add ROOT: " + t.root.data.toString()
				+ ": " + t.cString());
		...
		System.out.println("TestFromBook add: " + t.toString());
		...
		System.out.println("TestFromBook after remove abfd: " + t.toString());
	 ...
	 * @param nodes
	 *            the array into which to place the nodes
	 * @param start
	 *            the offset at which to start placing the nodes
	 * @return the number of nodes placed
	 */
	private static int getNodes(RedBlackTree.Node n, RedBlackTree.Node[] nodes,
			int start) {
		if (n == null) {
			return 0;
		}
	 ...
	 * 
	 * @param n
	 *            the root of the tree to reflect
	...
	public static void checkRedBlack(RedBlackTree t) {
		...
		for (int i = 0; i < nodes.length - 1; i++) {
			if (nodes[i].data.compareTo(nodes[i + 1].data) > 0) {
				throw new IllegalStateException(nodes[i].data
						+ " is larger than " + nodes[i + 1].data);
			}
		}
	}
	...
	private static int checkRedBlack(RedBlackTree.Node n, boolean isRoot) {
		...
		if (n.parent == null) {
			...
			if (n.color == RedBlackTree.RED
				...
				throw new IllegalStateException("Parent of red " + n.data
						+ " is red");
			}
		}
